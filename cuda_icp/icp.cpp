...
#endif
        for (int i = 0; i < static_cast<int>(model_pcd.size()); i++) {
...
#pragma omp parallel
                {
                    Vec16f reducer_private = Vec16f::Zero();
#pragma omp for nowait
#endif
                    for (int pcd_iter = 0; pcd_iter < (int)model_pcd.size(); ++pcd_iter) {
